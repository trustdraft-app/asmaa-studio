...
## 2026-06-12 — Asmaa.video (asmaa.video) — launch verifier brand token unblock
**Question:** CI Verify fails on every PR with "homepage/llms.txt missing Asmaa Studio" — shipped site says "Asmaa Video"; which side is stale?
**Decision:** Updated scripts/verify-launch.mjs brand tokens (homepage + llms.txt checks, duplicated-title guard) from "Asmaa Studio" to "Asmaa Video" to match shipped reality. No customer-facing copy was changed.
**Reasoning:** The owner's official packages PDF (PDF metadata Title "Asmaa Video", Author "Mohamed Alsaeed") and the PR #74/#76 owner-PDF-driven rebuild brand the product "Asmaa Video" across homepage, og siteName, and llms.txt — newest owner-sourced evidence supersedes the 2026-05-28 "rename to Asmaa Studio" note. The verifier was the only artifact still enforcing the old name and was failing every PR. Flagged for Mohammed: .agent/README.md still records the Studio rename — final brand name is a brand decision; if "Asmaa Studio" is wanted, the site copy (not the verifier) must change.
**Source:** public/packages-asmaa-studio.pdf metadata, app/layout.tsx siteName, out/ greps, CI run 27385254209, professional judgment.
